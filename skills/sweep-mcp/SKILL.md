---
name: sweep-mcp
description: Exhaustively E2E-test a whole MCP plugin's tool surface ("auf Herz und Nieren"). Use when the user wants a full sweep of an MCP, not a single-ticket check. Inspects the tool surface, decomposes it into stateful capability clusters, spawns one black-box cluster-tester subagent per cluster (parallel where independent, serial for live real-system MCPs like agent-vdesktop, fixtures where coupled), collects compact findings digests, and hands them to the file-findings skill. Optionally records repeatable test suites (record mode) for later zero-LLM replay via the deterministic runner. Does NOT decide which MCP is in scope — the user states that at invoke time.
---
...
- Independent, read-only tools (discovery, search, monitor/info, pipeline reads) each form their own light
  cluster — cheap, parallel, no setup.
- **Scale the decomposition to the surface size.** A large MCP -> one worker per cluster. A tiny MCP
  (e.g. a 3-tool worktree lifecycle) -> a single worker, or test it inline. **Do not go finer than a
  cluster:** a per-single-call worker has to rebuild the cluster's state from scratch, which costs *more*
  tokens, not fewer.

Reference clusters (a starting point, not a fixed list — derive from the actual surface, extend as the
ecosystem grows):

| MCP | Typical clusters |
|---|---|
| `agent-project-issues` | ticket-lifecycle / pr-lifecycle / pr-review / comments / relations / pipelines (read) / discovery (read) |
| `agent-vdesktop` | desktops / windows / window-state / layout+launch / pinning / query (read) / monitors (read) |
| `agent-worktree` | one worktree-lifecycle cluster |

`agent-vdesktop` is a **live real-system MCP** (it drives the user's real desktop) — sweep it under the
extra rules in §4b, not the parallel default.

## 3. Handle coupling between clusters

Some clusters depend on another entity existing (comments and relations need a ticket; PR-review needs a
PR; pipeline reads need something built). Two ways to handle it:

- **Fold** the dependency into one worker (e.g. let the ticket worker also touch comments), **or**
- **Seed a fixture**: create the prerequisite yourself (one `create_ticket` / `create_pr`) and pass its id
  into the dependent worker's `probe_focus`. Prefer this when you want the clusters tested independently and
  in parallel.
...
(e.g. ticket -> PR -> comment) is just a worker with a broader `tool_list` and a scripted `probe_focus` —
not a different kind of agent. **In record mode**, also pass `record: true` and a `server_ref` (the logical
handle you'll use for this MCP in the suite, e.g. `pi`).
...
## 4b. Live / stateful real-system MCPs (e.g. `agent-vdesktop`)

Some MCPs act on a **shared real resource the user is actively using** — the physical desktop, real
windows, even the terminal hosting this session. For these, two things change; everything else in §4 still
holds:

- **Serialize every cluster.** Spawn one worker, await its digest, then spawn the next — never a parallel
  batch, not even for the read-only clusters. Multiple agents racing one physical desktop cause desktop
  switches, window relocation, and singleton-app hijacks. Time is not the constraint here; safety is, and
  serial is fine.
- **Still delegate — do not drive the probing inline.** Whether a worker drives the desktop or you do, the
  physical desktop and this session's terminal are shared either way, so moving the probing into the
  orchestrator buys *no* safety — it only costs tokens. Serial means one worker at a time, not you calling
  the tools yourself.

The orchestrator owns the **safety baseline** so a worker that dies mid-mutation can't leave the desktop
broken:

- **Take ONE baseline snapshot up front** (e.g. `list_desktops` + `list_windows` + `get_current_desktop`)
  and keep it. Pass it into each worker's `probe_focus` as the ground-truth state to restore to. Reuse that
  same snapshot for every worker — do **not** re-list state in the orchestrator between workers on the happy
  path (that just reintroduces the bloat you were avoiding). Re-snapshot and remediate only if a worker
  reports it could not restore, or its run ended abnormally.
- In each worker's `probe_focus`, spell out the live-system rules (the `cluster-tester` agent also enforces
  them): only mutate/clean up what it created; **never `close_window` — or otherwise destroy — a window it
  did not itself create** (`release_window` to untrack instead); restore to the baseline and verify before
  returning.

Pass the MCP's **known hazards** into every worker explicitly — a worker has no memory of past runs and
cannot see your notes. For `agent-vdesktop`:

- `pin_app_all_desktops` / `unpin_app` are **app-wide by AUMID**: acting through one handle affects *every*
  window of that app — including this session's terminal (all Windows Terminal windows share one host
  process) — and can relocate unreferenced windows to another desktop. Test only with an immediate revert.
- `launch_app` **adopts a pre-existing singleton window** (Notepad, Calculator, UWP apps, browsers) instead
  of opening a fresh one, handing back a handle to the user's window *with their unsaved content*. Prefer
  `launch_terminal` for disposable test windows; treat any window the worker did not explicitly create as
  possibly the user's.
...
(`bug` / `documentation` / `enhancement` + `severity:*` + `e2e-test`), thematic grouping, and **sequential**
ticket creation all live there — do not duplicate those rules here. Pass each finding with its repro call,
observed vs. expected, the worker's severity and routing hint, and which MCP/cluster it came from.
...
2. **Assemble a full suite per cluster.** You — not the worker — own the parts that touch how the server is
   ...
   servers:
     <server_ref>:                 # the same handle you gave the worker
       ...
       env_passthrough: [<TOKENS>] # e.g. [GITHUB_TOKEN]; [] if none
...
**Do not record live real-system MCPs (`agent-vdesktop`) in v1.** A deterministic replay would drive the
real desktop with no LLM judgment guarding it. Sweep them the normal way (§4b) and file findings, but skip
the fragment/suite step for those clusters.

## 8. Report

Summarize: which MCP was swept, which clusters were covered (and any blocked), total findings by severity,
and — from `file-findings` — which tickets were created in which repos, with URLs. In record mode, also
list the suites persisted to `mcp-suites/`.
