# Build the agent-mcp-tester MCP server into a single-file binary.
#
...
# extensionless `bin/mcp-tester`; each per-OS build job emits its own
...
#   pwsh -File scripts/build.ps1 -Package    # also stage build/stage/agent-mcp-tester/
#
...
Invoke-Py -m PyInstaller mcp-tester.spec --clean --noconfirm
if ($LASTEXITCODE -ne 0) {
...
if ($IsWindows) {
    ...
    # If the lock turns out to be a running mcp-tester.exe (i.e. the dev's
    ...
    if (-not $copied) {
        $running = @(Get-Process -Name mcp-tester -ErrorAction SilentlyContinue)
        if ($running.Count -gt 0) {
            $procPids = ($running | ForEach-Object { $_.Id }) -join ", "
            Write-Host "    mcp-tester.exe is still running (PID: $procPids)." -ForegroundColor Yellow
            Write-Host "    A Claude Code session likely has the plugin's MCP server loaded."
            Write-Host "    Close it (or run '/mcp' and disconnect 'mcp-tester') and re-run the build."
            Write-Host "    To kill it now without that:   Stop-Process -Name mcp-tester -Force"
        }
...
# 7. Optional: stage build/stage/agent-mcp-tester/ for the assembly step in
...
if ($Package) {
    ...
    $stage = Join-Path $root "build/stage/agent-mcp-tester"
    ...
    # This plugin ships skills/ AND agents/ alongside the binary — stage both.
    if (Test-Path "skills") {
        Copy-Item -Recurse -Force "skills" $stage
    }
    if (Test-Path "agents") {
        Copy-Item -Recurse -Force "agents" $stage
    }
    Copy-Item -Force "README.md" $stage -ErrorAction SilentlyContinue
    Copy-Item -Force "LICENSE" $stage -ErrorAction SilentlyContinue
    Write-Host "    build/stage/agent-mcp-tester (this-OS payload only)"
}

Write-Step "Done."
Write-Host "bin/$ExeName is ready. plugin.json points at the extensionless 'bin/mcp-tester' so each OS auto-selects its native binary."
